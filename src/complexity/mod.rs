...
pub use metrics::{FileComplexity, ComplexitySummary, ComplexityLevel};
...
pub fn analyze_file_complexity(
    ...
    let max_cyclomatic = functions_with_complexity.iter().map(|f| f.cyclomatic).max().unwrap_or(0);
    ...
    let max_function_length = functions_with_complexity.iter().map(|f| f.lines).max().unwrap_or(0);
...
pub fn calculate_complexity_summary(file_complexities: &[metrics::FileComplexity]) -> ComplexitySummary {
