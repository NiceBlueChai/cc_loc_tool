...
impl Default for AppConfig {
    fn default() -> Self {
        Self {
            ...
            exclude_dirs: HashSet::from(["node_modules".to_string(), ".git".to_string(), "target".to_string()]),
            exclude_files: vec!["*.generated.*".to_string(), "moc_*".to_string(), "qrc_*".to_string()],
            selected_languages: Language::all().iter().map(|l| l.display_name().to_string()).collect(),
            theme: Theme::Light,
        }
    }
}
...
impl AppConfig {
    ...
    pub fn load() -> Result<Self> {
        ...
        if config_path.exists() {
            ...
            // 确保语言列表是有效的
            config.selected_languages.retain(|lang| {
                Language::all().iter().any(|l| l.display_name() == lang)
            });
    ...
    pub fn get_selected_languages(&self) -> Vec<Language> {
        ...
        self.selected_languages
            ...
            .filter_map(|lang_name| {
                Language::all().iter().find(|l| l.display_name() == lang_name).copied()
            })
            .collect()
    }
